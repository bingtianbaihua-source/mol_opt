...
from transform.block import BlockGraphTransform
import pandas as pd
import pickle
...
class Trainer:
    ...
    def setup_date(self, args):
        ...
        if os.path.exists(data_pkl_path):
            with open(data_pkl_path, 'rb') as f:
                data_pkl = pickle.load(open(data_pkl_path))
        else:
            data_pkl = None
        ...
        def construct_dataset(df, 
            ...
            indexed_df = df.loc[index]
            molecules = [row.SMILES for row in indexed_df.itertuples(index=False)]
            properties = [
                {key: getattr(row, key) for key in args.property} for row in indexed_df.itertuples(index=False)
            ]
    ...
    def _step(self, batch, train: bool):
        ...
        x_upd_core, Z_core = self.core_mol_embedding(batch)
        ...
        x_upd_core, Z_core = self.condition_embedding(\
                                x_upd_core, 
                                Z_core, 
                                cond, 
                                node2graph_core)
        
        loss_term = self.termination_prediction(
                                Z_core, 
                                y_term)
        ...
        loss_atom = self.atom_prediction(
            x_upd_core, edge_index_core, edge_attr_core, Z_core, Z_pos_block, node2graph_core, y_atom
        )
    ...
    def building_block_embedding(self, 
                                 block: Data|LongTensor, 
                                 train: bool
                                 ):
        if train:
    ...
    def building_block_priority(self, 
                                Z_core: GraphVector, 
                                Z_block: GraphVector, 
                                y_add: BoolTensor, 
                                sample_type: str
    ...
    def termination_prediction(self, Z_core: GraphVector, y_term: BoolTensor) -> LossScalar:
        ...
        return F.binary_cross_entropy_with_logits(logit_term, y_term.float())
    ...
    def condition_embedding(self,
        ...
        return self.model.condition_embedding(\
                                    x_upd_core,
                                    Z_core,
                                    cond,
                                    node2graph_core)
         
    def core_mol_embedding(self,
                           batch_core: Data
                           ):
        return self.model.core_molecule_embedding(batch_core)

    def standardize_condition(self, 
                              condition: dict[str, FloatTensor]
                              ):
